...
export const petsApi = {
  ...
  getPetInfo: async (petId: number) => {
    const response = await customFetch.get<Response<PetType>>(
  ...
  putPetInfo: async (petId: number, data: PetInfoUpdateSchema) => {
    ...
    const response = await customFetch.put<Response<PetType>>(
      `${API_ENDPOINTS.PET}/${petId}`,
      {
  ...
  deletePetInfo: async (petId: number) => {
    const response = await customFetch.delete<Response<string>>(
      `${API_ENDPOINTS.PET}/${petId}`,
    );
    return response.result;
  },
  ...
  postPetInfo: async (data: PetInfoFormSchema) => {
    const formData = petFormData(data);
    const response = await customFetch.post<Response<PetType>>(
      `${API_ENDPOINTS.PET}`,
      {
        body: JSON.stringify(formData),
      },
